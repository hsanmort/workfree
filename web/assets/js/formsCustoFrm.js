...
var $addPropLinkFrm = $('<hr><a href="#" class="add_prop_link_frm btn-u">Add a formation</a>');
var $newLinkLiFrm = $('<li></li>').append($addPropLinkFrm);

jQuery(document).ready(function() {

    // Get the ul that holds the collection of props
   var $collectionHolder = $('ul.frms');
    // add the "add a prop" anchor and li to the props ul
    $collectionHolder.append($newLinkLiFrm);
    // count the current form inputs we have (e.g. 2), use that as the new
    // index when inserting a new item (e.g. 2)

    $collectionHolder.data('index', $collectionHolder.find(':input').length);

    $addPropLinkFrm.on('click', function(e) {
        // prevent the link from creating a "#" on the URL
        e.preventDefault();
        // add a new prop form (see code block below)
        addpropForm($collectionHolder, $newLinkLiFrm);
...
function addpropForm($collectionHolder, $newLinkLiFrm) {
